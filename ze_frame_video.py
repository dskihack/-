...
def split_audio_file(file_path, chunk_duration=60):
    """将音频文件分割成60秒的片段"""
    try:
        # 根据文件扩展名处理不同格式
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # 百度API配置
        self.baidu_api_config = {
            'api_key': 'xxxxx',      # 替换为你的API Key
            'secret_key': 'xxxxx', # 替换为你的Secret Key
    ...
    def recognize_with_baidu(self, audio_file):
        ...
        try:
            # 1. 获取access token
            if (not self.baidu_api_config['token'] or 
                self.baidu_api_config['token_expire'] < datetime.now().timestamp()):
                self.get_baidu_token()
            
            # 2. 分割音频文件
            audio_chunks = split_audio_file(audio_file)
            ...
            for i, chunk in enumerate(audio_chunks):
                ...
                payload = {
                    "format": "wav",
                    "rate": self.baidu_api_config['rate'],
                    "channel": 1,
                    "cuid": "pyqt_speech_app",
                    "token": self.baidu_api_config['token'],
                    "dev_pid": self.baidu_api_config['dev_pid'],
    ...
    def get_baidu_token(self):
        ...
        try:
            auth_url = f"https://aip.baidubce.com/oauth/2.0/token?grant_type=client_credentials&client_id={self.baidu_api_config['api_key']}&client_secret={self.baidu_api_config['secret_key']}"
            response = requests.get(auth_url)
            result = response.json()
            
            if 'access_token' in result:
                self.baidu_api_config['token'] = result['access_token']
                self.baidu_api_config['token_expire'] = datetime.now().timestamp() + result['expires_in']
            else:
                raise Exception(f"获取token失败: {result}")
        except Exception as e:
            raise Exception(f"获取百度token失败: {str(e)}")
    ...
    def recognize_speech(self):
        ...
        # 检查音频格式是否支持
        supported_formats = ['wav', 'pcm']  # 百度API支持的格式
        for file in self.audio_files:
            if not file.split('.')[-1].lower() in supported_formats:
                QMessageBox.warning(self, "格式错误", f"不支持的音频格式: {file}\n请使用WAV或PCM格式")
                return
        ...
        for i, audio_file in enumerate(self.audio_files):
            # 更新状态
            self.audio_list_edit.setPlainText(f"正在处理 {i+1}/{len(self.audio_files)}: {audio_file}")
        ...
        for i, (file, text) in enumerate(zip(self.audio_files, self.recognized_texts)):
            ...
            self.recognized_list_table.setItem(i, 1, QTableWidgetItem(text[:50] + "..." if text else "识别失败"))
        
        # 恢复按钮状态
        self.recognize_btn.setEnabled(True)
        
        QMessageBox.information(self, "完成", f"语音识别完成!\n成功: {success_count}/{len(self.audio_files)}")
